...
export default async function Profile({ params }: { params: { id: string } }) {
    ...
    const replies = await client.thread.findMany({
        where: {
            parentId: { not: null },
            author: params.id,
        },
    ...
    return (
        <section className="text-light-1">
            ...
            <ProfileBody threads={threads} replies={replies} isUser={session_data?.user.id == params.id ? true : false} follow={follow}></ProfileBody>
        </section>
    );
}